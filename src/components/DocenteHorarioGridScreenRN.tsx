    ...
    if (mode === "delete" && franja) {
      ...
      void (async () => {
        ...
        if (c0 >= f0 && c1 <= f1) {
          ...
          await eliminarClaseDocente(c.id);
        } else if (c0 >= f0 && c0 < f1) {
          // Toca el inicio
          await actualizarClaseDocente(c.id, { ...c, horaInicio: franja.horaFinal });
        } else if (c1 > f0 && c1 <= f1) {
          // Toca el final
          await actualizarClaseDocente(c.id, { ...c, horaFinal: franja.horaInicio });
        } else {
          // Está en el medio, partimos en dos
          await actualizarClaseDocente(c.id, { ...c, horaFinal: franja.horaInicio });
          await registrarClaseDocente({
            materia: c.materia,
            dia: c.dia,
