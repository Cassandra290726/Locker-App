...
import { fetchDocenteNotas, guardarCategoriasApi } from "@/lib/notasClient";
import {
  FILTRO_TODAS_ID,
  formatoFechaNota,
  previewNota,
...
export default function DocenteNotasScreenRN({ onBack }: Props) {
  ...
  const reload = useCallback(async () => {
    const d = await fetchDocenteNotas();
    setData(d);
  }, []);
  ...
  async function hechoCategorias() {
    setSavingCats(true);
    const ok = await guardarCategoriasApi(draftCats);
    setSavingCats(false);
    if (ok) {
      await reload();
      setCatModal(false);
